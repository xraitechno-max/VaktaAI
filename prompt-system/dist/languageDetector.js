...
export class LanguageDetector {
    MIN_CHARS = 6; // From policy
    CONFIDENCE_THRESHOLD = 0.75; // From policy
    HYSTERESIS_THRESHOLD = 0.65; // From policy
    lastDetectedLanguage = null;
    /**
     * Detect language from user message
     */
    detect(text, preferredLang) {
        ...
        if (text.length < this.MIN_CHARS) {
            const defaultLang = (preferredLang === "hi" || preferredLang === "hinglish" ? preferredLang : "english");
            return {
                label: defaultLang,
                confidence: 0.5,
                detected_from: text,
     ...
     */
    applyHysteresis(newLanguage, newConfidence) {
