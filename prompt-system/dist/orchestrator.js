...
import { extractCitations } from "./utils/citations.js";
import { extractFormulasWithUnits } from "./utils/units.js";
import { validateTaskInput } from "./utils/validation.js";
import { withSpan, addSpanEvent } from "./telemetry/otel.js";
import { recordResponse, recordLanguageDetection, recordRAGRetrieval, recordModelCall, recordCitationValidation, regenTotal, activeRequests, } from "./telemetry/metrics.js";
export class Orchestrator {
     ...
     */
    async run(task) {
        return await withSpan("orchestrator.run", {
            mode: task.mode,
            ...
            try {
                ...
                // 2. Detect language
                const langDetection = await withSpan("lang.detect", { text_length: task.user_msg.length, requested_lang: task.lang || "auto" }, async () => languageDetector.detect(task.user_msg, task.lang));
                const targetLanguage = langDetection.should_switch ? langDetection.label : "english";
                ...
                // 3. Route to model
                const routingDecision = await withSpan("router.decide", {
                    mode: task.mode,
                ...
                const evidence = await withSpan("rag.retrieve", { mode: task.mode, subject: task.subject }, async () => toolPlanner.executePlan(task));
                // Record RAG metrics
                recordRAGRetrieval(task.mode, task.subject, evidence.total_retrieved, evidence.chunks.reduce((sum, c) => sum + (c.similarity || 0), 0) / evidence.total_retrieved || 0);
                logger.info("Evidence retrieved", {
                    chunks: evidence.total_retrieved,
                    sufficient: evidence.has_sufficient_evidence,
                });
                // 5. Build prompt
                const promptOutput = await withSpan("prompt.build", { mode: task.mode, language: targetLanguage }, async () => promptBuilder.build(task, evidence, targetLanguage));
                ...
                while (regenerationCount <= this.MAX_REGENERATIONS) {
                    // Generate draft
                    draft = await withSpan("llm.generate", {
                        model: currentModel,
                        mode: task.mode,
                        attempt: regenerationCount + 1,
                    }, async () => this.generateDraft(promptOutput.messages, currentModel, routingDecision.temperature, routingDecision.max_tokens, task.mode, regenerationCount > 0 ? verifierReport : undefined));
                    // 7. Run acceptance gate
                    verifierReport = await withSpan("gate.verify", { mode: task.mode, attempt: regenerationCount + 1 }, async () => acceptanceGate.verify(draft, targetLanguage, task.mode, regenerationCount));
                    ...
                    // Record regeneration
                    const regenReasons = [];
                    if (!verifierReport.gates.fact_check.passed)
                        regenReasons.push("fact_unsupported");
                    if (!verifierReport.gates.math_check.passed)
                        regenReasons.push("math_fail");
                    if (!verifierReport.gates.language_check.passed)
                        regenReasons.push("lang_mismatch");
                    if (verifierReport.confidence_score < 0.72)
                        regenReasons.push("low_conf");
                ...
                if (!verifierReport.overall_pass) {
                    if (regenerationCount >= this.MAX_REGENERATIONS) {
                        ...
                        return this.createErrorResult("MAX_REGENERATIONS_EXCEEDED", `Failed to generate acceptable answer after ${this.MAX_REGENERATIONS} attempts. Last confidence: ${verifierReport.confidence_score}`, startTime);
                    }
                    activeRequests.labels(task.mode).dec();
                    return this.createErrorResult("VERIFICATION_FAILED", `Answer did not pass verification gates. Confidence: ${verifierReport.confidence_score}`, startTime);
                }
                ...
                recordCitationValidation(task.mode, task.subject, verifierReport.gates.fact_check.passed, verifierReport.gates.fact_check.passed ? undefined : "missing_or_invalid");
                // Record final response metrics
                recordResponse(task.mode, task.subject, targetLanguage, currentModel, verifierReport.confidence_score, regenerationCount, totalLatency);
     ...
     */
    async generateDraft(messages, model, temperature, maxTokens, mode, previousVerification) {
        ...
        recordModelCall(model, attempt, response.latency_ms, response.usage.prompt_tokens, response.usage.completion_tokens);
        const draft = {
     ...
     */
    createFinalAnswer(draft, task, language, verification, model, evidence) {
        // Extract citations
        const citations = extractCitations(draft.raw_text).map((cit) => {
            const chunk = evidence.chunks.find((c) => c.citation === cit);
            return {
                citation_id: cit,
     ...
     */
    createMockDraft(mode) {
        const mockText = mode === "explain"
